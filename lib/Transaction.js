...
var FEE_PER_1000B_SAT = parseInt(0.001 * util.COIN);
...
TransactionOut.prototype.isNull = function isNull() {
  return buffertools.compare(this.v, util.INT64_MAX) === 0;
};

TransactionOut.prototype.setNull = function setNull() {
  this.v = util.INT64_MAX;
  this.s = util.EMPTY_BUFFER;
};


Transaction.POW_VERSION = 1;
function Transaction(data) {
  if ("object" !== typeof data) {
    data = { timestamp: timeUtil.curtime() };
  }
...
Transaction.prototype.isCoinStake = function() {
  return this.ins.length > 0 && (!this.ins[0].isCoinBase()) &&
    this.outs.length >=2 && this.outs[0].isNull();
};
...
Transaction.prototype.serialize = function serialize() {
  ...
  if (this.version > Transaction.POW_VERSION)
  {
    buf = new Buffer(8);
...
TransactionSignatureSerializer.prototype.serialize = function() {
  ...
  // Cannacoin doesn't include nTime in the serialization for signatures
  // serialize nTime
  //if (this.txTo.version > Transaction.POW_VERSION)
  //  this.bytes.word32le(this.txTo.timestamp);
};
...
Transaction.prototype.getStandardizedObject = function getStandardizedObject() {
  ...
  var totalSize = 12; // version + lock_time + timestamp
...
Transaction.prototype.fromObj = function fromObj(obj) {
  ...
  txobj.timestamp = obj.timestamp || 0;
  txobj.ins = [];
  txobj.outs = [];
  ...
  this.timestamp = txobj.timestamp;
  this.ins = txobj.ins;
