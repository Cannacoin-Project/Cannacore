...
var cannacoreDefaults = require('../config');
...
function PeerManager(config) {
  ...
  this.config = config || {};
  for (var i in cannacoreDefaults)
    if (cannacoreDefaults.hasOwnProperty(i) && this.config[i] === undefined)
      this.config[i] = cannacoreDefaults[i];
